...
namespace OAR_API::Animations
{
	IAnimationsInterface* GetAPI(const InterfaceVersion a_interfaceVersion /*= InterfaceVersion::Latest*/)
	{
		...
		const auto requestAPIFunction = pluginHandle ? reinterpret_cast<_RequestPluginAPI_Animations>(GetProcAddress(pluginHandle, "RequestPluginAPI_Animations")) : nullptr;
